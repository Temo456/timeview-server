...
(async function () {
  ...
  const chapters = [
    ['00:00–02:00','引导与开场','earth','先看引导视频，再找一座熟悉的城市。','大家好，我是AI解说阿远。今天从地球的时间地图出发，认识太阳系，再观察一个大家选择的历史日期。','我是AI解说阿星。我们会提出几个小问题，你可以点选答案，也可以选择日期。先看看眼前的地球，能找到你熟悉的城市吗？'],
    ...
    ['12:00–18:00','时间互动 · 两道题','earth','先选择时差问题的答案，再选择日期对照。','刚才我们比较了三个城市的时间。接下来，想请你也一起观察这幅图。','不用急着回答。我们一次只看一个问题，点选符合你想法的选项就好。'],
    ...
    ['44:00—47:00','生日天象示范','solar','演示日期：2000-01-01，12:00，北京时间。','我们查看所选日期的天象与历法信息，形成一份可核对的记录，不作个人评价或未来预测。','先看底部日期，再看图景，最后看报告。只提供日期时，采用北京时间十二点。'],
    ...
    ['57:00–60:00','回到现在','solar','恢复当前时间，选择下次想探索的主题。','今天练习了读取城市时间，认识季节、轨道和空间方向，也观察了特定日期的天象记录。','下一次你想继续看世界时间、太阳系，还是更远的星空？你可以点选一个主题，作为下一堂课的参考。']
  ];
  ...
  function cleanSolar() { scene().reset?.(); scene().focus?.('planets'); }
  function enterChapter(step) {
    window.TimeviewCards?.hide();
    window.TimeviewCourseVisuals?.hide();
    $('tvReport').hidden=true;
    scene().reset?.();
    // Restore the current chapter's visual context even when its opening lines have no cue.
    if (step.scene) return; // The current line applies its cue immediately below.
    const chapterSteps=steps.filter(s=>s.ch===step.ch);
    const previous=steps.slice(0,cursor).filter(s=>s.ch===step.ch&&s.scene).pop();
    if(previous){applyScene(previous.scene);return;}
    const first=chapterSteps.find(s=>s.scene)?.scene;
    if(first&&!first.startsWith('planet:')){applyScene(first);return;}
    if(view==='solar') {
      if(chapterSteps.some(s=>(s.type==='date'||s.type==='example')&&s.birthday))scene().report?.();
      else scene().focus?.('planets');
    }
  }
  ...
  for (let ch = 0; ch < chapters.length; ch++) {
    ...
    if (ch === 3) {
      ask(ch, '先看一看，北京、伦敦和纽约显示的是同一天吗？请选择你的观察。', '同一瞬间，不同城市可能处在不同的日期。我们一起核对画面上的年月日，再比较小时和分钟。', ['是同一天','不是同一天','还不确定']);
      date(ch, '我们换一天试试看。你想看哪个日期？选择公历年月日就可以。', '2000-01-01', false);
    }
    if (ch === 4) {
      ask(ch, '北半球是夏天的时候，南半球也一样吗？', '南北半球的季节相反。地轴倾斜，让两边获得日照的情况随公转发生变化。', ['一样，也是夏天','不一样，是冬天','还不确定']);
      ...
      ask(ch, '北京在夏至附近，通常比冬至附近白昼更长吗？', '北京在夏至附近通常比冬至附近白昼更长。北京位于北半球，地轴倾斜使这里在夏季通常有更长的白昼。', ['白昼更长','白昼更短','一样长','还不确定']);
    ...
    if (ch === 10) {
      date(ch,'现在轮到你了。可以选择一个生日，我们一起看看那一天。只选择日期也可以，统一用北京时间中午十二点作演示。','2000-01-01',true);
      date(ch,'我们再看一个日期。还有哪位观众愿意分享？','2024-06-21',true);
    }
    if (ch === 11) {
      say(ch,0,'现在让时间恢复流动，回到今天。',()=>{scene().now();report=null;$('tvReport').hidden=true;});
      ask(ch,'下一次你想继续了解世界时间、太阳系，还是更远的星空？请选择一个主题。','我们今天先到这里。以后可以沿着这些方向，继续认识时间与宇宙。',['世界时间','太阳系','更远的星空']);
    }
  }
  ...
  try {
    ...
    if(!script){
      if(courseId!=='cosmos-basics')throw Error('script unavailable');
      const response=await fetch('course-default.json',{cache:'no-store',signal:AbortSignal.timeout(5000)});
      if(!response.ok)throw Error('script unavailable');
      script=await response.json();
    }
    if(script.schemaVersion!==2||!script.chapters?.length||!script.lines?.length)throw Error('script mismatch');
    chapters.splice(0,chapters.length,...script.chapters.map(ch=>['',ch.title,ch.view]));
  ...
  async function waitReply(step,gen){
    ...
    const area=$('tvReplyArea'),prompt=$('tvPrompt'),isDate=step.type==='date';
    area.replaceChildren();area.hidden=false;prompt.hidden=false;panel.classList.add('replying');
    ...
    return new Promise(resolve=>{
      ...
      const choices=document.createElement('div');choices.className='reply-choices';area.append(choices);
      const button=(label,action)=>{const b=document.createElement('button');b.type='button';b.className='reply-choice';b.textContent=label;b.onclick=action;choices.append(b);return b;};
      if(isDate){
        const today=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Shanghai',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date());
        const part=name=>today.find(p=>p.type===name).value;
        const todayValue=part('year')+'-'+part('month')+'-'+part('day');
        button('演示日期 '+step.fallback,()=>end(step.fallback));
        if(parseDate(todayValue))button('今天',()=>end(todayValue));
        const form=document.createElement('form');form.setAttribute('aria-label','选择其他日期');
        const row=document.createElement('div');row.className='date-parts';form.append(row);
      ...
      const skip=document.createElement('button');skip.type='button';skip.className='skip-reply';skip.textContent=isDate?'跳过，使用演示日期':'跳过，继续听';skip.onclick=()=>end(null);area.append(skip);
      arm(isDate?replySettings.dateReplySeconds:replySettings.replySeconds);tick=setInterval(draw,250);
      const body=$('tvBody');
      if(isDate)body.scrollTop=body.scrollHeight;
      else body.scrollTop+=$('tvLog').lastElementChild.getBoundingClientRect().top-body.getBoundingClientRect().top-8;
    });
  }
  ...
  function updateStage(ch){$('tvStage').value=ch;$('tvStageLabel').textContent=(ch+1)+' / '+chapters.length+' · '+chapters[ch][1];$('tvCues').style.setProperty('--progress',(chapters.length>1?ch/(chapters.length-1)*100:100)+'%');$('tvCues').querySelectorAll('button').forEach((b,i)=>{b.classList.toggle('past',i<ch);if(i===ch)b.setAttribute('aria-current','step');else b.removeAttribute('aria-current');});}
  ...
  $('tvRate').onchange=()=>{const rate=Number($('tvRate').value);if(!speechRates.includes(rate))return;speechRate=rate;if(activeAudio){activeAudio.defaultPlaybackRate=rate;activeAudio.playbackRate=rate;}try{localStorage.setItem(rateKey,String(rate));}catch(_){}};
